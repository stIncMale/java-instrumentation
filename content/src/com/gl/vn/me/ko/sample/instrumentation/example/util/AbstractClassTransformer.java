...
public abstract class AbstractClassTransformer implements ClassFileTransformer {
	...
	public AbstractClassTransformer(final Set<String> classesToTransform, final boolean defrostAllowed) {
		...
		for (final String classToTransform : classesToTransform) {
			final String internalRepresentationClassToTransform = classToTransform.replace(JavassistEnvironment.PACKAGE_SEPARATOR,
					JavassistEnvironment.INTERNAL_PACKAGE_SEPARATOR);
	...
	public final byte[] transform(final ClassLoader classLoader, final String className, final Class<?> classBeingRedefined,
	...
	private final byte[] unsafeTransform(final ClassLoader classLoader, final String className, final Class<?> classBeingRedefined,
			final ProtectionDomain protectionDomain, final byte[] classfileBuffer) throws IllegalClassFormatException {
		final byte[] transformedBytes;
		if (acceptClassForTransformation(className)) {
			...
			if (classLoader == null) {// bootstrap class loader
				...
				ctClass = JavassistEnvironment.getCtClassOrNull(className);// try faster way of
																			// getting CtClass
				if (ctClass == null) {
				...
				if (defrostAllowed) {
					...
					if (logger.isDebugEnabled()) {
						logger.debug("Class '" + className
								+ "' can't be transformed, because the corresponding CtClass is frozen and defrost is not allowed");
					}
