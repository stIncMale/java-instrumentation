 ...
 * It means that the class is not applicable for cases when classes with the same name are loaded with different class loaders
 * (don't be disoriented by the method {@link #getCtClass(ClassLoader, String)}).
 * <p>
 * The class provides an ability to obtain an exclusive access to its methods, see {@link #lock()} and {@link #unlock()} methods.
 * <p>
 * Note also that {@link javassist.CtClass} is not thread-safe,
 * so all operations with the class must be explicitly synchronized in case of concurrent access. Unfortunately it's impossible to hide the
 * underlying {@link javassist.ClassPool} object behind the {@link com.gl.vn.me.ko.sample.instrumentation.util.javassist.JavassistEnvironment} class,
 * because {@link javassist.CtClass#getClassPool()}
 ...
 * {@link com.gl.vn.me.ko.sample.instrumentation.util.javassist.JavassistEnvironment} there is no guarantee that no one else use the same
 * {@link javassist.CtClass} object.
 * <p>
...
public final class JavassistEnvironment {
  ...
  /**
   * All public methods, except {@link JavassistEnvironment#lock()}/{@link JavassistEnvironment#unlock()},
   * must acquire {@link JavassistEnvironment#SHARED_LOCK} in the beginning and release this lock before completion.
   */
  private final static Lock SHARED_LOCK;
   ...
   *
   * @param classPath The class path to append. Must be not {@code null}.
   *
   * @return <ul>
   ...
   *
   * @param ctClass Class object to convert. Must be not {@code null}.
   *
   * @return The contents of the class file.
   *
   * @throws javassist.CannotCompileException When bytecode transformation has failed.
  ...
  public final static byte[] getCtBytes(final CtClass ctClass) throws CannotCompileException {
    ...
    try {
      ...
      try {
        ...
        throw new RuntimeException("Exception occurred while trying to get bytecode from class '" + ctClass.getName() + "'", e);
      }
    } finally {
      SHARED_LOCK.unlock();
    }
  ...
  /**
   * Acts just like {@link #getCtClass(ClassLoader, String)} considering that supplied instance of {@link java.lang.Class}
   * have information about defining class loader and class name.
   *
   * @param clazz Class to search. Must be not {@code null}.
   *
   * @return A {@link javassist.CtClass} object representing specified class.
   *
   * @throws javassist.NotFoundException When the requested class can't be find in the underlying {@link javassist.ClassPool} object.
   ...
   * This method is equivalent to {@link #getCtClass(String)} except that it tries to get {@link javassist.CtClass} object using known class path
   * (see {@link #getCtClassOrNull(String)}) and in case
   * of failure it appends class path obtained from the provided class loader and tries to get the class once again (see {@link #getCtClass(String)}).
   * In other words the method does something like<br>
   * <blockquote>
   *
   * <pre>
   * {
   ...
   * </blockquote>
   *
   * @param classLoader The defining loader of the class (should be {@code null} if the bootstrap loader).
   * @param className A fully-qualified class name. Must be not {@code null}.
   *
   * @return A {@link javassist.CtClass} object representing specified class.
   *
   * @throws javassist.NotFoundException When the requested class can't be find in the underlying {@link javassist.ClassPool} object.
   ...
   * <p>
   * If class name ends with {@code "[]"}, then this method returns a {@link javassist.CtClass} object for that array type.
   * To obtain an nested class, use {@code "$"} instead of {@code "."} for separating the enclosing class name and the nested class name.
   * <p>
   * Returned object always have pruning turned off.
   *
   * @param className A fully qualified class name (for example {@code "java.lang.Object"} or {@code "java/lang/Object"}). Must be not {@code null}.
   *
   * @return A {@link javassist.CtClass} object representing specified class.
   *
   * @throws javassist.NotFoundException When the requested class can't be find in the underlying {@link javassist.ClassPool} object.
  ...
  /**
   * Reads a class file from the source and returns a reference to the {@link javassist.CtClass} object representing that class file.
   * This method is equivalent to {@link #getCtClass(String)}
   * except that it returns {@code null} when a class file is not found and it never throws an exception.
   *
   * @param className A fully-qualified class name. Must be not {@code null}.
   *
   * @return A {@link javassist.CtClass} object representing specified class or {@code null}.
   *
   * @see #getCtClass(String)
   */
  @Nullable
   ...
   * If a thread tries to invoke a method (except {@link #unlock()}) of {@link JavassistEnvironment} class (even without trying to acquire the lock)
   * and the lock is held by another thread,
   * then current thread becomes disabled for thread scheduling purposes and lies dormant until lock has been released.
   * <p>
   * Acquires the lock if the lock aren't held by another thread and returns immediately.
   * The lock is reentrant so if the current thread already holds the lock then the hold count is incremented by
   * one and the method returns immediately.
   * <p>
   * If one need to obtain exclusive access to {@link JavassistEnvironment}, one can lock the environment,
   * do required actions and unlock the environment. The following idiom should be used
   * <blockquote>
   *
   * <pre>
   ...
   *
   * @param preserveClassPath Specifies if class path elements prepended/appended via methods {@link #prependClassPath(ClassPath)},
   * {@link #appendClassPath(ClassPath)} must be presented
  ...
  /**
   * Provides an ability to control access to {@link JavassistEnvironment} class methods.
   * Attempts to release the lock. If the current thread is the holder of the lock then the hold count is decremented.
