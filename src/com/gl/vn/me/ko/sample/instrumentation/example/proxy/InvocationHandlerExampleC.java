...
/**
 * This class is an invocation handler that is used by {@link com.gl.vn.me.ko.sample.instrumentation.example.proxy.MapProxyFactoryExampleC}
 * to create Java-proxy objects.
 * <p>
 * Instantiability: allowed.<br>
 * Mutability: this class itself has nothing that can cause mutability; depends on the object that was supplied to constructor
 * {@link #InvocationHandlerExampleC(Map)}.<br>
 * Thread safety: this class itself has nothing that can cause thread unsafety; depends on the object that was supplied to constructor
 * {@link #InvocationHandlerExampleC(Map)}.
 */
final class InvocationHandlerExampleC implements InvocationHandler {
  ...
  @Nullable
  private final static Object invokeOriginalMethod(final Object proxiedObject, final Method method, @Nullable final Object[] methodArgs) throws
   ...
   *
   * @param object An original object that can be hided behind the proxy so invocation handler is the only way to access it. Must be not {@code null}.
   */
  ...
  /**
   * Invokes provided method on the proxied (original) object. If name of the method to invoke is {@code "get"}
   * and the only argument of the method is equal to object that denotes hot or cold faucet
   * valves (see {@link com.gl.vn.me.ko.sample.instrumentation.example.ExampleC}),
   * then the invocation becomes tricky:the value of argument is swapped from hot to cold and vice versa.
   *
  ...
  public final Object invoke(
      ...
      @Nullable final Object[] methodArgs) {
    if (METHOD_NAME_TO_TRICK.equals(method.getName()) && (methodArgs != null) && (methodArgs.length == 1)) {
      ...
      if (argumentSwapped) {
        ...
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug(
              "Method '" + method.toString() + "' was invoked with argument '" + originalMethodArg + "' (" + valveToString(originalMethodArg) +
                  "). Swap it with '" + newMethodArg
    ...
    try {
      ...
      throw new RuntimeException(
          "Proxied method '" + method.toString() + "' invoked on object '" + proxiedObject.toString() + "' have thrown an exception",
          e);
    }
    return result;
  }
