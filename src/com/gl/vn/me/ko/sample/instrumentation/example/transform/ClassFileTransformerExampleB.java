...
/**
 * Transforms {@link com.gl.vn.me.ko.sample.instrumentation.example.ExampleB} class by modifying all its declared non-native methods
 * according to the following algorithm:<br>
 * <ul>
 * <li>for every declared non-native method in {@link com.gl.vn.me.ko.sample.instrumentation.example.ExampleB}
 * search for {@link java.math.BigDecimal} constructor calls</li>
 * <li>substitute any found call of {@link java.math.BigDecimal} constructor with call of
 * {@link com.gl.vn.me.ko.sample.instrumentation.example.proxy.BigDecimalProxyFactoryExampleB}{@code .create(...)}
...
public class ClassFileTransformerExampleB extends AbstractClassFileTransformer {
  ...
  /**
   * Returns {@code true} only if {@code className} is equal to {@code "com/gl/vn/me/ko/sample/instrumentation/example/ExampleB"}.
  ...
  protected byte[] doTransform(final CtClass ctClass) throws Exception {
    ...
    synchronized (ctClass) {
      if (ctClass.isFrozen()) {
        ...
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug(
              "Substituting constructor calls 'new " + originalClass.getName() + "(...)' with the method '" + substitutionalClass.getName() + "." +
                  METHOD_NAME_TO_SUBSTITUTE_FOR
                  + "(...)' invocations");
        }
