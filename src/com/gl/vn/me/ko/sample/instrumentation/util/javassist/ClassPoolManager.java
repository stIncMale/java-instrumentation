...
final class ClassPoolManager {
  /**
   * Modifications of {@link #classPool}, {@link #preserveClassPath}, {@link #PREPENDED_CLASSPATH_ELEMENTS}, {@link #APPENDED_CLASSPATH_ELEMENTS}
   * must be synchronized via this object.
   */
  ...
  // must be volatile because there are reads of the field outside synchronized blocks (see getClassPool() method)
  ...
  final static boolean appendClassPath(final ClassPath classPath) {
    ...
    return modifyClassPath(classPool, classPath, true);
  }
   ...
   *
   * @return An instance of {@link javassist.ClassPool}.
   *
   ...
   *
   * @param classPath The class path to prepend. Must be not {@code null}.
   *
   * @return <ul>
  ...
  final static boolean prependClassPath(final ClassPath classPath) {
    ...
    return modifyClassPath(classPool, classPath, false);
  }

  /**
   * Invocation of the method guarantees creation of a new instance of {@link javassist.ClassPool} the next time method
   * {@link #getClassPool()} will be called.
   *
   * @param preserveClassPath Specifies if prepended/appended class path elements should be presented in a new instance of
   * {@link javassist.ClassPool}.
   *
   * @see #getClassPool()
   */
